...
class PayService():
    ...
    def createOrder(self, member_id, items=None, parms=None):
        ...
        for item in items:
            ...
            pay_price = decimal.Decimal(pay_price) + decimal.Decimal(item['price']) * int(item['number'])
        ...
        yun_price = parms['yun_price'] if parms and 'yun_price' in parms else 0
        note = parms['note'] if parms and 'note' in parms else ''
        ...
        try:
            ...
            for item in items:
                ...
                if int(item['number'])>int(tmp_left_stock):
                    raise Exception('您购买的美食太火爆了，剩余：%s，您购买：%s')%(tmp_left_stock,tmp_left_stock)
