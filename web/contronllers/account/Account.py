...
from common.models.model import User,AppAccessLog
...
def index():
    ...
    if 'mix_kw' in req:
        rule = or_(User.nickname.like("%{0}%".format(req['mix_kw'])),User.mobile.like("%{0}%".format(req['mix_kw'])))
...
def info():
    ...
    if uid < 1:
        return redirect(UrlManager.buildUrl('account/index.html'))
